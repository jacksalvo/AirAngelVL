...
//			long id_camera, int venderId, int productId, int fileDescriptor, int busNum, int devAddr, String usbfs
			...
			throw new IllegalStateException("Unable to open the USB camera connection", e);
		}

		if (result != 0) {
			mCtrlBlock.close();
			mCtrlBlock = null;
			throw new UnsupportedOperationException("Native USB camera open failed: " + result);
		}
