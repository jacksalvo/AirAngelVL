...
//		if (DEBUG) XLogWrapper.v(TAG, "getDeviceKeyName:" + sb.toString());
	...
	private static String getString(final UsbDeviceConnection connection, final int id, final int languageCount, final byte[] languages) {
		...
		String result = null;
		// String descriptors are optional metadata. Bound discovery time and decode
		// the little-endian language ID after the two-byte descriptor header.
		for (int i = 0; i < Math.min(languageCount, 2); i++) {
			final int index = 2 + i * 2;
			if (index + 1 >= languages.length) break;
			final int language = (languages[index] & 0xff) | ((languages[index + 1] & 0xff) << 8);
			int ret = connection.controlTransfer(
				USB_REQ_STANDARD_DEVICE_GET, // USB_DIR_IN | USB_TYPE_STANDARD | USB_RECIP_DEVICE
				USB_REQ_GET_DESCRIPTOR,
				(USB_DT_STRING << 8) | (id & 0xff), language, work, 256, 250);
			if ((ret > 2) && (ret <= work.length) && ((work[0] & 0xff) == ret) && (work[1] == USB_DT_STRING)) {
		...
		if (device != null) {
			if (BuildCheck.isLollipop()) {
				...
				try {
					...
					// Permission can disappear during unplug; serial is optional.
				}
			}
			if (BuildCheck.isMarshmallow()) {
			...
			if ((manager != null) && manager.hasPermission(device)) {
				...
				if (desc == null || desc.length < USB_DT_DEVICE_SIZE) return info;
				...
				try {
					int result = connection.controlTransfer(
						USB_REQ_STANDARD_DEVICE_GET, // USB_DIR_IN | USB_TYPE_STANDARD | USB_RECIP_DEVICE
	    				USB_REQ_GET_DESCRIPTOR,
	    				(USB_DT_STRING << 8) | 0, 0, languages, 256, 250);
					if (result > 0) {
					...
					// Descriptor failures do not prevent using the already-open device.
				}
				} catch (RuntimeException ignored) {
					// Unplug can invalidate descriptor/serial reads at any point.
				} finally {
					connection.close();
				}
			}
	...
	public static final class UsbControlBlock implements Cloneable {
		...
		private UsbControlBlock(final USBMonitor monitor, final UsbDevice device) {
			...
			if (mConnection == null) {
				throw new IllegalStateException("Device was removed or permission was revoked");
			}
			...
			if (v != null && v.length >= 2) {
				try {
					...
					// Device names are supplied by the USB service, not a public format.
				}
			}
			mBusNum = busnum;
			mDevNum = devnum;
		...
		public synchronized void close() {
			...
			if (mConnection != null) {
				...
				if (monitor != null) {
					...
					if (device != null) {
						// UVCCamera clones this block. Closing a clone must not forget
						// the original connection still owned by the monitor.
						monitor.mCtrlBlocks.computeIfPresent(device,
							(key, current) -> current == UsbControlBlock.this ? null : current);
					}
				}
			}
		}
